 ...
 * @author Haifeng Li
 */
public class SpikedMultivariateGaussianDistribution extends AbstractMultivariateDistribution
	 ...
	 *            variance.
	 */
	public SpikedMultivariateGaussianDistribution(double[] mean, double var) {
		if (var <= 0) {
	 ...
	 *            covariance matrix.
	 */
	public SpikedMultivariateGaussianDistribution(double[] mean, double[][] cov) {
	...
	public SpikedMultivariateGaussianDistribution(double[] mean, double[][] precision, boolean inverse) {
	 ...
	 *            the training data.
	 */
	public SpikedMultivariateGaussianDistribution(double[][] data) {
		this(data, false);
	}
	 ...
	 *            the training data.
	 * @param diagonal
	 *            true if covariance matrix is diagonal.
	 */
	public SpikedMultivariateGaussianDistribution(double[][] data, boolean diagonal) {
	...
	private void init() {
		...
		try {
			...
			Jama.Matrix sigMat = new Matrix(sigma);
			EigenvalueDecomposition evd = sigMat.eig();
			Matrix D = evd.getD();
			Matrix V = evd.getV();
			Matrix S = new Matrix(D.getRowDimension(),D.getColumnDimension());
			for (int i = 0; i < D.getColumnDimension(); i++) {
				if(D.get(i, i) >= 0 &&Math.abs(D.get(i, i))>=1.0e-8)
					S.set(i, i, Math.sqrt((D.get(i, i))));
				else
					S.set(i, i, 0);
				System.out.println("the no. " + i + " eig-recovered\t" + S.get(i, i));
			}
			Matrix choL = V.times(S);
			sigmaDet = Math.sqrt(Math.abs(sigMat.det()));
			sigmaL = choL.getArrayCopy();
	...
	/**
	 * Algorithm from Alan Genz (1992) Numerical Computation of Multivariate
	 * Normal Probabilities, Journal of Computational and Graphical Statistics,
	 * pp. 141-149.
	 *
	 * The difference between returned value and the true value of the CDF is
	 * less than 0.001 in 99.9% time. The maximum number of iterations is set to
	 * 10000.
	 */
	@Override
	...
	public MultivariateMixture.Component M(double[][] x, double[] posteriori) {
		...
		c.priori = alpha;
		SpikedMultivariateGaussianDistribution g = new SpikedMultivariateGaussianDistribution(mean, cov);
		g.diagonal = diagonal;
		c.distribution = g;
