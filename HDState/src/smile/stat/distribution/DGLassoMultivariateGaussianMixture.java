...
import smile.stat.distribution.DGLassoMultivariateGaussianDistribution;
 ...
 * @author Haifeng Li
 */
public class DGLassoMultivariateGaussianMixture extends MultivariateExponentialFamilyMixture {

    /**
     * Constructor.
     * @param mixture a list of multivariate Gaussian distributions.
     */
    public DGLassoMultivariateGaussianMixture(List<Component> mixture) {
        super(mixture);
    }
     ...
     */
    public DGLassoMultivariateGaussianMixture(double[][] data, int k) {
        this(data, k, false);
    }
     ...
     */
    public DGLassoMultivariateGaussianMixture(double[][] data, int k, boolean diagonal) {
        if (k < 2)
        ...
        c.priori = 1.0 / k;
        DGLassoMultivariateGaussianDistribution gaussian = new DGLassoMultivariateGaussianDistribution(centroid, sigma);
        ...
        for (int i = 1; i < k; i++) {
            ...
            c = new Component();
            c.priori = 1.0 / k;
            gaussian = new DGLassoMultivariateGaussianDistribution(centroid, sigma);
            gaussian.diagonal = diagonal;
            c.distribution = gaussian;
            components.add(c);
        }
     ...
     * @param data the training data.
     */
    public DGLassoMultivariateGaussianMixture(double[][] data) {
        this(data, false);
    }
    ...
    public DGLassoMultivariateGaussianMixture(double[][] data, boolean diagonal) {
        if (data.length < 20)
        ...
        Component c = new Component();
        c.priori = 1.0;
        c.distribution = new DGLassoMultivariateGaussianDistribution(data, diagonal);
        mixture.add(c);
    ...
    private void split(List<Component> mixture) {
        ...
        for (Component c : mixture) {
            double sigma = ((DGLassoMultivariateGaussianDistribution) c.distribution).scatter();
            if (sigma > maxSigma) {
                maxSigma = sigma;
                componentToSplit = c;
            }
        ...
        double[][] delta = ((DGLassoMultivariateGaussianDistribution) componentToSplit.distribution).cov();
        double[] mu = ((DGLassoMultivariateGaussianDistribution) componentToSplit.distribution).mean();
        ...
        c.distribution = new DGLassoMultivariateGaussianDistribution(mu1, delta);
        mixture.add(c);
        ...
        c.distribution = new DGLassoMultivariateGaussianDistribution(mu2, delta);
        mixture.add(c);
