 ...
 * @author Haifeng Li
 */
public class DGLassoMultivariateGaussianDistribution extends AbstractMultivariateDistribution implements MultivariateExponentialFamily {
     ...
     * @param var variance.
     */
    public DGLassoMultivariateGaussianDistribution(double[] mean, double var) {
        if (var <= 0) {
     ...
     */
    public DGLassoMultivariateGaussianDistribution(double[] mean, double[][] cov) {
        if (mean.length != cov.length) {
            throw new IllegalArgumentException("Mean vector and covariance matrix have different dimension");
        }
		cov=Estimator.inverse(new NonSparseEstimator()._deSparsifiedGlassoPrecisionMatrix(cov));
    ...
    public DGLassoMultivariateGaussianDistribution(double[] mean, double[][] precision, boolean inverse) {
     ...
     */
    public DGLassoMultivariateGaussianDistribution(double[][] data) {
        this(data, false);
    }
     ...
     */
    public DGLassoMultivariateGaussianDistribution(double[][] data, boolean diagonal) {
        ...
        if (diagonal) {
    		...
    		sigma=Estimator.inverse(new NonSparseEstimator()._deSparsifiedGlassoPrecisionMatrix(sigma));
        }
    ...
    public MultivariateMixture.Component M(double[][] x, double[] posteriori) {
        ...
        c.priori = alpha;
        DGLassoMultivariateGaussianDistribution g = new DGLassoMultivariateGaussianDistribution(mean, cov);
        g.diagonal = diagonal;
        c.distribution = g;
