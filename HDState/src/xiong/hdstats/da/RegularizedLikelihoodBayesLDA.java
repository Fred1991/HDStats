...
public class RegularizedLikelihoodBayesLDA extends GLassoLDA {
	...
	public RegularizedLikelihoodBayesLDA(double[][] d, int[] g, int size, int K) {
		super(d, g, false);
		...
		Matrix covarianceSqrt = CholeskyDecompositionMTJ.create(
	            DenseMatrixFactoryMTJ.INSTANCE.copyMatrix(mf.copyArray(super.pooledInverseCovariance).inverse())).getR();
		...
		while (pooledClassifiers.size() < size) {
			...
			MultivariateGaussianDistribution gaussian = new MultivariateGaussianDistribution(super.globalMean, marray,false);
	...
	private double numPredictors=-1;
	...
	public int predict(double[] x) {
		...
		if(obtainedClassifiers==null)
			obtainedClassifiers=this.pooledClassifiers;
		...
		for (int label : probX.keySet()) {
			for (double[][] pm : probX.get(label).keySet()) {
				...
				sumExpLogProbs.put(label, sumExpLogProbs.get(label)+(Math.exp(probX.get(label).get(pm) - maxValues.get(label))));
			}
		}
	...
	public static void main(String[] args) {
		...
		RegularizedLikelihoodBayesLDA test = new RegularizedLikelihoodBayesLDA(data, group, 1000, 1);
