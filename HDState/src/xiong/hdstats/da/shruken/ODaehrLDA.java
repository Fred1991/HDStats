...
public class ODaehrLDA implements Classifier<double[]> {
	...
	public ODaehrLDA(double[][] d, int[] g, boolean p) {
		...
		// for(int i=0;i<subset.length;i++){
		// new
		// DiagKeptSparseCovEstimator(Estimator.lambda,10).covarianceApprox(covariance[i]);
		// }
		...
		new DiagKeptSparseCovEstimator(CovarianceEstimator.lambda, 5).covarianceApprox(pooledInverseCovariance);

		try {
			pooledInverseCovariance = new Matrix(pooledInverseCovariance).inverse().getArray();
		} catch (Exception e) {
			System.err.println("singular in ODaehr");
			pooledInverseCovariance = PseudoInverse.inverse(new Matrix(pooledInverseCovariance)).getArray();
		}

		/// pooledInverseCovariance=new
		/// Matrix(pooledInverseCovariance).inverse().getArray();
