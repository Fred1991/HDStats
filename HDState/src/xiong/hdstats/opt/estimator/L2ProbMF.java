...
public class L2ProbMF {
	...
	public L2ProbMF(Matrix _R, double _lp, double _lq) {
		this.R = _R;
		this.lambdaP = _lp;
		this.lambdaQ = _lq;
	}
	...
	public static class ProbMFPRiskFunction implements RiskFunction {
		private L2ProbMF mf;

		public ProbMFPRiskFunction(L2ProbMF _mf) {
			this.mf = _mf;
		}
	...
	public static class ProbMFQRiskFunction implements RiskFunction {

		private L2ProbMF mf;

		public ProbMFQRiskFunction(L2ProbMF _mf) {
			this.mf = _mf;
		}
	...
	public static ChainedFunction getProbMFRiskFunction(Matrix R, double _lp, double _lq) {
		L2ProbMF mf = new L2ProbMF(R, _lp, _lq);
		List<RiskFunction> lrf = new ArrayList<RiskFunction>();
		lrf.add(new ProbMFPRiskFunction(mf));
		lrf.add(new ProbMFQRiskFunction(mf));
		ChainedRiskFunction crf = new ChainedRiskFunction(lrf);
		return crf;
	}

	public static ChainedMVariables initiProbMFPQ(Matrix R, int latent) {
	...
	public static void main(String[] args) throws IOException {
		...
		int i = 30;
		ChainedFunction cf = getProbMFRiskFunction(toM, 0, 0);
		ChainedMVariables cmv = initiProbMFPQ(toM, i);
		ChainedMVariables res = GradientDescent.getMinimum(cf, cmv, 10e-14, 10e-4, 1000, GradientDescent.GD);
		Matrix P = getP(res);
		Matrix Q = getQ(res);
