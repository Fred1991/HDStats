...
public class TruncatedEVD {
	...
	//	System.out.println(npe);
		...
		mtx[0] = D.getMatrix(D.getRowDimension()-k, D.getRowDimension() - 1, 
							 D.getRowDimension()-k, D.getRowDimension() - 1).copy();
		
		numPositiveEigs(mtx[0]);
		
	//	mtx[1] = VT.transpose();
		
		mtx[1] = VT.getMatrix(D.getRowDimension()-k, D.getRowDimension() - 1, 
							  0, VT.getColumnDimension() - 1).copy().transpose();
		return mtx;
	}
	...
	public static void main(String[] args) {
		...
		System.out.println(
				...
				+ "Sample");
		System.out.println("" 
				+ Utils.getErrorL2(cov, estCov) + "\t"
				+ Utils.getErrorL2(cov, estCov2) + "\t"
				+ Utils.getErrorL2(cov, estCov3) + "\t"
				+ Utils.getErrorL2(cov, gCov)+"\t"
				+ Utils.getErrorL2(cov, dgCov)+"\t"
				+ Utils.getErrorL2(cov, samCov)+"\n"
				
				+ Utils.getErrorL2(graph, graphEst) + "\t"
				+ Utils.getErrorL2(graph, graphEst2) + "\t"
				+ Utils.getErrorL2(graph, graphEst3) + "\t"
				+ Utils.getErrorL2(graph, gLasso) + "\t"
				+ Utils.getErrorL2(graph, dgLasso) + "\t"
				+ Utils.getErrorL2(graph, graphSam)+"\n"
