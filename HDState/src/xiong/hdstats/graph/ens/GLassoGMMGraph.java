...
public class GLassoGMMGraph extends SampleCovarianceEstimator{
	...
	public GLassoGMMGraph(double[][] data, int size) {
		this.size = size;
		this.nData=data.length;
		this.dimensions=data[0].length;
		this.gmm=new GLassoMultivariateGaussianMixture(data,size);
	...
	public int[][] thresholdingDiff(double t, double e, GLassoGMMGraph wg) {
		List<int[][]> tGraphs = new ArrayList<int[][]>();
		for (int i = 0; i < this.size; i++)
			tGraphs.add(sampledGraphs.get(i).thresholding(t));
	...
	public int[][] adaptiveThresholdingDiff(double t, double overlap, GLassoGMMGraph wg) {
		List<int[][]> tGraphs = new ArrayList<int[][]>();
		for (int i = 0; i < this.size; i++)
			tGraphs.add(sampledGraphs.get(i).adaptiveThresholding(t));
		List<int[][]> wGraphs = new ArrayList<int[][]>();
